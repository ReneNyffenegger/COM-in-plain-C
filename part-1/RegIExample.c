...
int WINAPI WinMain(HINSTANCE hinstExe, HINSTANCE hinstPrev, LPSTR lpszCmdLine, int nCmdShow) {
    ...
    LSTATUS regStatus;
		...
		// Open "HKEY_LOCAL_MACHINE\Software\Classes"
		if (! (regStatus = RegOpenKeyEx(HKEY_LOCAL_MACHINE, &ClassKeyName[0], 0, KEY_WRITE, &rootKey))) {

      result = 2;
			// Open "HKEY_LOCAL_MACHINE\Software\Classes\CLSID"
			if (! (regStatus = RegOpenKeyEx(rootKey, &CLSID_Str[0], 0, KEY_ALL_ACCESS, &hKey) )) {
        result = 3;
				// Create a subkey whose name is the ascii string that represents
				// our IExample2 object's GUID
				stringFromCLSID(&buffer[0], (REFCLSID)(&CLSID_IExample));

//      MessageBox(0, buffer, "stringFromCLSID", 0);

				if (! (regStatus = RegCreateKeyEx(hKey, &buffer[0], 0, 0, REG_OPTION_NON_VOLATILE, KEY_WRITE, 0, &hKey2, &disposition))) {
          result = 4;
					...
					if (! (regStatus = RegCreateKeyEx(hKey2, &InprocServer32Name[0], 0, 0, REG_OPTION_NON_VOLATILE, KEY_WRITE, 0, &hkExtra, &disposition))) {
            result = 5;
						if (! (regStatus = RegSetValueEx(hkExtra, 0, 0, REG_SZ, (const BYTE *)&filename[0], lstrlen(&filename[0]) + 1))) {
							...
							if (! (regStatus = RegSetValueEx(hkExtra, &ThreadingModel[0], 0, REG_SZ, (const BYTE *)&BothStr[0], sizeof(BothStr)))) {
								result = 0;
		...
		if (result) {
			cleanup();
      char buf[200];
      wsprintf(buf, "Failed to register IExample.DLL as a COM component. (Error: %d, regStatus = %d - 5 means: access denied)", result, regStatus);
			MessageBox(0, buf, &ObjectDescription[0], MB_OK|MB_ICONEXCLAMATION);
		}
	}
